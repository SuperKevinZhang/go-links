...
gameDetail.loadData=function(id,winflag){
		// start loading
...
$(function(){
	...
	$("#smallImage").bind("vclick",function(){
		$("#gameDetail_selectPic").toggle();
	});
	
	//拍照
	$("#gameDetail_pic").bind("vclick",function(){
		console.log('拍照: ' );

		capturePhoto();
	});
	//本地选择
	$("#gameDetail_file").bind("vclick",function(){
		console.log('选择照片: ' +pictureSource.SAVEDPHOTOALBUM);
		getPhoto(pictureSource.SAVEDPHOTOALBUM);
	});
	
	/*//上传照片
	$("#gameDetail_upload").bind("vclick",function(){
		uploadimage(params.id);
	});*/
	

	$("#gameDetail_back").bind("vclick",function(){
		// 后退
		javascript:history.back(-1);	
	});
     //gameDetail_selectPic
	// click comfirm button
	$("#gd_a_gameComfirm").bind("vclick",function(){
		    // comfirm game
			gameDetail.comfirmGame();
		});

	//load data
    gameDetail.loadData(params.id,params.winflag);
